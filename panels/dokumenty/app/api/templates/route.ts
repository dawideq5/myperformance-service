...
import { listTemplates, uploadPdfTemplate } from "@/lib/documenso";
...
export async function POST(req: Request) {
  ...
  const { name, pdfBase64, folderName } = body ?? {};
  if (typeof name !== "string" || typeof pdfBase64 !== "string") {
    return NextResponse.json({ error: "Brak wymaganych pól" }, { status: 400 });
  }

  try {
    const result = await uploadPdfTemplate({ name, pdfBase64, folderName });
    broadcast({
