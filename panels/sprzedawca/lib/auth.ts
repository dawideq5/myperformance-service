...
export const REQUIRED_ROLE = "sprzedawca";
const PANEL_CLIENT_ID = "panel-sprzedawca";
...
export const authOptions: AuthOptions = {
  ...
  callbacks: {
    async jwt({ token, account }) {
      // Initial sign-in — kapturzuj tokeny z account.
      if (account) {
      ...
      // Jeśli access token nadal ważny (z buforem 30s) → bez akcji.
      ...
      // Refresh access token przez Keycloak. Bez tego po ~5 min wszystkie
      // /api/relay/* dostają 401 bo accessToken wygasł.
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(`KC refresh ${res.status}`);
        }
