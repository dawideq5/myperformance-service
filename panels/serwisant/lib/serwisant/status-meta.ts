...
/**
 * Lokalna definicja `ServiceStatus`. Backend (root `lib/services.ts`) jest
 * źródłem prawdy i może być rozszerzany przez agenta backendowego —
 * dlatego klucze w `STATUS_META` używamy jako `string`, a `getStatusMeta`
 * akceptuje dowolny string i fallbackuje do `neutral`. Dzięki temu Twój
 * panel nie wywali się gdy pojawi się nowa wartość po stronie backendu.
 */
