...
// Trzymaj oba bajt-identyczne (poza ewentualnymi różnicami ścieżek). Wzorzec
// odpowiada istniejącemu duplikatowi PhoneConfigurator3D w obu panelach —
// każdy panel ma własny `output: standalone` Next.js build i własne
// node_modules, więc cross-panel imports się nie buildują (drei/fiber/three
// nie znalazłyby się w resolution graph z `panels/shared/`). Patrz
// docs/wave1/F12-intake-diff.md sekcja "Decyzja architektoniczna".
//
// Zmiana w tym pliku → skopiuj do drugiego panelu (lub odwrotnie).
...
} from "./PhoneConfigurator3D";
import {
  DescriptionPicker,
  deserializeRepairTypes,
  serializeRepairTypes,
  useRepairTypes,
} from "./DescriptionPicker";
import { QuotePreview } from "./QuotePreview";

/**
 * Wave 22 / F12 — ujednolicony formularz przyjęcia serwisowego.
 *
 * Obie role (sprzedawca, serwisant) widzą TEN SAM formularz. Różnice tylko
 * w opcjonalnych sekcjach + post-submit flow:
 *
 *   mode="sales"
 *     - sekcja "Punkt serwisowy" (sprzedawca wybiera dokąd zlecenie idzie)
 *     - sekcja "Kanał kodu wydania" (email/SMS/papier)
 *     - po sukcesie redirect do `/serwis/${id}` (panel sprzedawcy) — chyba
 *       że `onCreated` zwróci `true` (parent przejmuje nawigację).
 *
 *   mode="service"
 *     - sekcja "Punkt serwisowy" UKRYTA (serwisant *jest* punktem serwisowym)
 *     - sekcja "Kanał kodu wydania" UKRYTA (sales-only flow z Wave 21/Faza 1C)
 *     - po sukcesie wywołanie `onCreated(service)` zamiast redirectu — parent
 *       (np. modal w PanelHome) decyduje co dalej.
 *
 * Wszystkie pozostałe sekcje (urządzenie, blokada, stan wizualny 3D,
 * opis usterki + wycena, dane klienta, potwierdzenie odbioru/handover)
 * są wspólne — to jest cel ujednolicenia: serwisant nie ma uproszczonego
 * widoku, dostaje pełną ścieżkę intake.
 */
export interface AddServiceFormProps {
  /** ID punktu (sprzedaży lub serwisu) — auto-prefilled przy POST. */
  locationId: string;
  /** "sales" = sprzedawca (default), "service" = serwisant. */
  mode?: "sales" | "service";
  /** Edycja istniejącego serwisu — pobierze detail + wypełni formularz. */
  editingServiceId?: string | null;
  /** Callback po anulowaniu edycji (sales mode) lub po anulowaniu intake. */
  onEditDone?: () => void;
  /** Callback wywołany po pomyślnym utworzeniu zlecenia.
   *  Gdy zwróci `true`, formularz NIE robi redirectu — parent jest
   *  odpowiedzialny za nawigację (np. modal close + auto-select).
   *  Gdy zwróci `false`/`undefined` w mode="sales", użyjemy domyślnego
   *  redirectu do `/serwis/${id}`. */
  onCreated?: (service: {
    id: string;
    ticketNumber: string;
    [k: string]: unknown;
  }) => boolean | void;
  /** Komunikaty błędów walidacji (np. niedozwolona kombinacja napraw).
   *  Domyślnie alert(). Sprzedawca podpina toast.push. */
  onError?: (params: { title: string; message: string }) => void;
  /** Sales-only — opcjonalne handlery do przycisków "Otwórz potwierdzenie" /
   *  "Wyślij elektroniczne". Gdy nie podane, success block (dead-code path
   *  bo redirect przed nim) nie pokazuje akcji. */
  receiptHandlers?: {
    openReceipt: (
      serviceId: string,
...
export function AddServiceForm({
  ...
  // Fallback do alert() gdy parent nie podpiął onError — pozwala formularzowi
  // działać w panelu bez ToastProvider (serwisant). Sprzedawca przekazuje
  // toast.push przez `onError` prop.
  ...
  useEffect(() => {
    // Service mode nie wybiera punktu serwisowego (jest nim sam serwisant) —
    // pomijamy fetch który expects sales-locationId po stronie backendu.
    if (isService) return;
    let alive = true;
    void (async () => {
      try {
  ...
  // Sekwencja sekcji — "service" pomijamy w mode="service" (sekcja Punkt
  // serwisowy jest ukryta). Wpływa na continueToNext + scroll-into-view.
  const SECTION_ORDER = (
    isService
      ? ["device", "lock", "visual", "description", "customer", "handover"]
      : [
          "device",
          "lock",
          "visual",
  ...
  // Sales-only: punkt serwisowy musi być wybrany przed handoverem.
  // Service-mode: serwisant *jest* punktem serwisowym — zawsze "complete".
  const serviceComplete = isService ? true : chosenServiceLocationId != null;
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (combinationError) {
      reportError("Niedozwolona kombinacja napraw", combinationError);
      return;
    }
    setSaving(true);
    setError(null);
    ...
    try {
      ...
      const body = {
        locationId,
        // Sales-only: sprzedawca wybiera punkt serwisowy (destination).
        // Service-mode: serwisant *jest* punktem serwisowym, backend mappuje
        // automatycznie po locationId — nie przekazujemy serviceLocationId
        // żeby nie nadpisać auto-mapping.
        ...(isSales ? { serviceLocationId: chosenServiceLocationId } : {}),
        type: "phone",
        ...
        // Sales-only — service mode nie wystawia kodu wydania przy intake.
        ...(isSales && releaseCodeChannel ? { releaseCodeChannel } : {}),
      };
      ...
      // formularz zatrzymuje się bez redirectu.
      const handled = onCreated?.(json.service) === true;
      if (handled) {
        return;
      }
      if (isSales) {
        // Sales mode: domyślny flow — redirect do widoku zlecenia,
        // user decyduje co robić (drukuj/wyślij/edytuj).
        const target = `/serwis/${serviceId}${significant ? "?resign=1" : ""}`;
        window.location.href = target;
        return;
      }
      // Service mode bez `onCreated` — pokazujemy success message,
      // parent prawdopodobnie zamknie modal po swojemu.
      setSuccess(`Zlecenie ${ticketNumber} utworzone.`);
      return;
    } catch (err) {
              ...
              <div className="mt-3 pt-3 border-t border-emerald-500/20">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  <button
                    type="button"
                    onClick={async () => {
                      const r = await receiptHandlers.sendElectronicReceipt(
                        lastCreated.id,
                        lastCreated.handover,
                      );
                      if (r.ok) {
        ...
        >
          <div className="space-y-3">
            ...
            {/* Wave 21 / Faza 1C — kanał kodu wydania urządzenia.
                Sales-only: serwisant przyjmuje urządzenie u siebie i nie
                wystawia kodu wydania przy intake. */}
            {isSales && !editingServiceId && (
