...
import { listServiceDocuments } from "@/lib/service-documents";

/**
 * Wave 21 / Faza 1B — biblioteka dokumentów per zlecenie.
 *
 * GET zwraca aktywne (deleted_at IS NULL) dokumenty zlecenia, posortowane
 * od najnowszych. Auth + ownership po location_id (panel-auth helper).
 */
...
export async function GET(
  ...
  const documents = await listServiceDocuments(id);
  return NextResponse.json({ documents }, { headers: PANEL_CORS_HEADERS });
}
