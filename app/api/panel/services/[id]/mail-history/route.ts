...
import { listMessagesForRecipient } from "@/lib/postal";
...
export async function GET(
  ...
  const email = service.contactEmail ?? "";
  if (!email) {
    return NextResponse.json(
      { messages: [] },
  ...
  const messages = await listMessagesForRecipient(email, 30);
  return NextResponse.json({ messages }, { headers: PANEL_CORS_HEADERS });
}
