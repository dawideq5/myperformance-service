...
export async function GET(request: Request) {
  try {
    ...
    const users: AdminUserSummary[] = rawUsers.map((u) => ({
      ...
      createdTimestamp: (u.createdTimestamp as number) ?? null,
      requiredActions: Array.isArray(u.requiredActions)
        ? (u.requiredActions as string[])
        : [],
    }));
  ...
  try {
    ...
    if (sendEmail && newId) {
      try {
        ...
        log.warn("admin.users.invite.email_failed", {
          userId: newId,
    ...
    log.info("admin.users.invite", {
      userId: newId,
      email,
      areaRoles: areaRoles.length,
      roleAssignmentErrors: roleAssignmentErrors.length,
    });
