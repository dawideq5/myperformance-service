...
function SmtpConfigsPanel() {
  ...
  const [ovhDomains, setOvhDomains] = useState<{ name: string; mailboxCount: number }[]>([]);
  const [ovhMailboxes, setOvhMailboxes] = useState<OvhMailboxBrief[]>([]);
  const [ovhLoading, setOvhLoading] = useState(false);
  const [selectedOvhDomain, setSelectedOvhDomain] = useState<string | null>(null);
  ...
  async function loadOvhMailboxes(domain: string) {
    ...
    try {
      ...
      setError(err instanceof ApiRequestError ? err.message : "Mailboxes load failed");
    } finally {
      setOvhLoading(false);
    }
  }
      ...
      <div className="flex flex-wrap gap-2">
        <Button
          onClick={async () => {
            ...
            setOvhLoading(true);
            try {
              const r = await api.get<{ domains: { name: string; mailboxCount: number }[] }>(
                  ...
                  {ovhMailboxes.map((mb) => (
                    ...
                    >
                      ...
                      <div className="flex gap-1">
                        {mb.isBlocked && <Badge tone="danger">zablokowana</Badge>}
