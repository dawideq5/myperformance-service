import { getServerSession } from "next-auth/next";
import { redirect } from "next/navigation";
import { authOptions } from "@/app/auth";
import { canAccessAdminPanel } from "@/lib/admin-auth";
import { CorrespondenceClient } from "./CorrespondenceClient";

export const dynamic = "force-dynamic";

/**
 * Strona `/admin/correspondence` — agregator korespondencji email + Chatwoot
 * dla zweryfikowanych skrzynek OVH. Trzy-kolumnowy layout z lewa: skrzynki,
 * środek: lista wątków, prawa: detail.
 */
export default async function AdminCorrespondencePage() {
  const session = await getServerSession(authOptions);

  if (!session?.user || session.error === "RefreshTokenExpired") {
    redirect("/login");
  }
  if (!canAccessAdminPanel(session)) {
    redirect("/forbidden");
  }

  return (
    <CorrespondenceClient
      userLabel={session.user.name ?? session.user.email ?? ""}
      userEmail={session.user.email ?? undefined}
    />
  );
}
