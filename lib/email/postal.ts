...
import { withExternalMysql } from "@/lib/db";
 ...
 *
 * Wszystkie zapytania idą przez `withExternalMysql` — wspólny pool +
 * dev graceful-degrade gdy POSTAL_DB_URL pokazuje docker-internal hostname.
 */

function withPostal<T>(fn: (p: mysql.Pool) => Promise<T>): Promise<T> {
  return withExternalMysql("POSTAL_DB_URL", fn);
}
...
export async function updateServer(
  ...
  values.push(id);
  await withPostal((p) =>
    p.execute(`UPDATE servers SET ${sets.join(", ")} WHERE id = ?`, values),
  );
}

export async function deleteServer(id: number): Promise<void> {
  await withPostal((p) =>
    p.execute(`UPDATE servers SET deleted_at = NOW() WHERE id = ?`, [id]),
  );
}
...
export async function deleteCredential(id: number): Promise<void> {
  await withPostal((p) => p.execute(`DELETE FROM credentials WHERE id = ?`, [id]));
}
