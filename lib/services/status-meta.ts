/**
 * Wave 22 / F7 — single source of truth dla metadanych statusów zlecenia
 * serwisowego (ServiceStatus). Wcześniej duplikat istniał w
 * `panels/serwisant/lib/serwisant/status-meta.ts` i
 * `panels/sprzedawca/lib/serwisant/status-meta.ts`. Te pliki w panelach
 * pozostają jako thin re-exporty z lokalnego mirrora
 * (`panels/<x>/lib/services/status-meta.ts`), dzięki czemu panele zostają
 * niezależnymi appami Next.js (każdy ma własny `node_modules`), ale meta
 * statusów synchronizuje się 1:1 z tym plikiem.
 *
 ...
 * UI nie wybuchało gdy backend doda nowy status.
 */
...
/**
 * Zwraca jedynie polską etykietę dla statusu — wygodne dla `event-humanizer`,
 * któremu icon/tone nie są potrzebne (a przekazywanie ReactNode'ów do
 * pure-string layeru jest zbędne).
 */
